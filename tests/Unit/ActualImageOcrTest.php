...
class ActualImageOcrTest extends TestCase
{
    ...
    public function test_cyprus_point_front_nine_ocr_extraction()
    {
        $imagePath = base_path('tests/scorecards/cyprus-point-front.jpg');
        $this->assertFileExists($imagePath, 'Cyprus Point Front scorecard image must exist');
        ...
        $this->analyzeGolfData($ocrResults['raw_text'], 'Front Nine (Holes 1-9)');
        ...
        // Assertions
        $this->assertNotEmpty($ocrResults['raw_text'], 'OCR should extract text from the image');
        $this->assertGreaterThan(0, $ocrResults['confidence'], 'OCR should have some confidence level');
        $this->assertIsArray($ocrResults['words'], 'OCR should return word-level data');
        $this->assertIsArray($ocrResults['lines'], 'OCR should return line-level data');
    }

    public function test_cyprus_point_back_nine_ocr_extraction()
    {
        $imagePath = base_path('tests/scorecards/cyprus-point-back.jpg');
        $this->assertFileExists($imagePath, 'Cyprus Point Back scorecard image must exist');
        ...
        // Analyze for golf-specific data
        echo "⛳ Golf Data Analysis:\n";
        $this->analyzeGolfData($ocrResults['raw_text'], 'Back Nine (Holes 10-18)');
    ...
    public function test_image_preprocessing_both_scorecards()
    {
        echo "\n".str_repeat('=', 80)."\n";
        ...
        foreach ($images as $filename => $description) {
            ...
            $this->assertFileExists($imagePath, "{$filename} must exist");
            ...
            $this->assertIsArray($corners);
            $this->assertArrayHasKey('top_left', $corners);
            $this->assertArrayHasKey('top_right', $corners);
            $this->assertArrayHasKey('bottom_left', $corners);
            $this->assertArrayHasKey('bottom_right', $corners);
        }
    ...
    private function analyzeGolfData(string $text, string $section): void
    {
